...
class UserGroup(Group):
    ...
    users = models.ManyToManyField("users.User", related_name="user_groups", blank=True)
    tenant = models.ForeignKey("clients.Tenant", on_delete=models.CASCADE, related_name="user_groups", blank=True, null=True)
    description = models.TextField(blank=True, null=True)
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    
    class Meta:
        verbose_name = t("User Group")
        verbose_name_plural = t("User Groups")
        ordering = ["-created_at"]
...
class User(AbstractUser):
    ...
    tenant = models.ForeignKey("clients.Tenant", on_delete=models.CASCADE, related_name="users", blank=True, null=True)
...
class UserRateLimitFlag(models.Model):
    uid = models.UUIDField(default=uuid.uuid4, editable=False, unique=True)
    user = models.ForeignKey("users.User", on_delete=models.CASCADE, related_name="rate_limit_flags")
